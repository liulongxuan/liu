...
for (const i in list) {
  ...
  //常用数据核心(有到期时间)
  if (list[i].hx.indexOf('hxpda') != -1) {  chxm1023["receipt"]["in_app"] = (sjbl); liulongxuan["latest_receipt_info"] = (sjbl); liulongxuan["pending_renewal_info"] = [{  "product_id" : (list[i].id), "original_transaction_id" : "490001314520000", "auto_renew_product_id" : (list[i].id), "auto_renew_status" : "1"  }]; chxm1023["latest_receipt"] = (list[i].latest);  }
  //永久数据核心(无到期时间)
